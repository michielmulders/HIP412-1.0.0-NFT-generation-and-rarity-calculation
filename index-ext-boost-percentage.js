/* This version also supports boolean-based and percentage-based attributes
 * For percentage-based attributes, we use the same keys-values format 
 * Ignoring "boost", "date", "datetime", and "color" type attributes as they are considered esthetical or not contributing directly to rarity ranking (it's up to NFT collections to determine if they want these traits to influence rarity by modifying the calculation algorithm)
 * This version uses config: nft-config-ext-percentage.json
 * Output is written to: /outputs-ext-percentage
 */

const fs = require("fs");

const baseSchema = require("./base-schema.json");
const nftConfig = require("./nft-config-ext-percentage.json");
let copyNFTConfig = JSON.parse(JSON.stringify(nftConfig));
const outputDir = 'outputs-ext-percentage';
...
//
// Check 3: percentage type should only contain keys that are of type number
console.log(`\n\n>> Validate NFT config`);
nftConfig.attributes.map((attribute) => {
  ...
  // Check 3
  if (attribute.display_type && attribute.display_type === "percentage") {
    const onlyNumbers = attribute.keys.every(key => (typeof key === 'number'));

    if (!onlyNumbers) {
      console.log(
        `Trait ${attribute.trait_type} of type percentage should only contain numbers.`
      );
      process.exit(1)
    }
  }
